<section class="section">
    <div class="container">
        ...
        <form method="GET" action="<?= e(url('/performance')) ?>" class="performance-filters-bar" id="performance-filter-form">
            <div class="filter-group filter-buttons">
                <?php foreach (['7d' => '7 days', '30d' => '30 days', '90d' => '90 days', 'all' => 'All time'] as $key => $label): ?>
                    <button type="submit" name="range" value="<?= $key ?>" class="filter-btn <?= (($range ?? 'all') === $key) ? 'is-active' : '' ?>">
                        <?= $label ?>
                    </button>
                <?php endforeach; ?>
            </div>
        ...
        <div class="stat-grid three" style="grid-template-columns: repeat(auto-fit, minmax(220px, 1fr));">
            <?= component('stat-card', ['label' => 'Win rate', 'value' => $stats['win_rate'] . '%', 'meta' => $stats['wins'] . 'W - ' . $stats['losses'] . 'L - ' . $stats['pushes'] . 'P']) ?>
